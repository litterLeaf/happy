...
public class LivenessDetectionMainActivity extends LoadingActivity implements ViewUpdateEventHandlerIf {
    ...
    private NewCircularCountDownProgressBar mOliveappCountDownProgressbar; // 动作提示倒计时
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mActionChangeMode = INSTANT_CHANGE;
...
//            actionBar.hide();
        ...
        mOliveappCountDownProgressbar = (NewCircularCountDownProgressBar) findViewById(getResources().getIdentifier("oliveapp_step_countdown_progressbar", "id", mPackageName));
                        ...
                        try {
                            ...
                            if (flastActionResult == 1000) {
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        mOliveappStepHintText.setText(getString(R.string.oliveapp_goodjob));
                                    }
                                });
        ...
        try {
            ...
            switch (newActionType) {
                case FacialActionType.UNKNOWN:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_normal", "string", mPackageName));
                    break;
                case FacialActionType.MOUTH_OPEN:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_mouthopen", "string", mPackageName));
                    break;
                case FacialActionType.EYE_CLOSE:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_eyeclose", "string", mPackageName));
                    break;
                case FacialActionType.HEAD_LEFT:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_headleft", "string", mPackageName));
                    break;
                case FacialActionType.HEAD_RIGHT:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_headright", "string", mPackageName));
                    break;
                case FacialActionType.HEAD_UP:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_headup", "string", mPackageName));
                    break;
                case FacialActionType.HEAD_SHAKE_SIDE_TO_SIDE:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_headshake", "string", mPackageName));
                    break;
                default:
                    hintText = getString(getResources().getIdentifier("oliveapp_step_normal", "string", mPackageName));
            }
