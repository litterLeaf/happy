...
public class ChatRowRobotMenu extends ChatRow{
    ...
    protected void onSetUpView() {
        ...
        if((info = MessageHelper.getRobotMenu(message)) != null){
            if(info.getTitle().equals("请选择以下菜单")){
                tvTitle.setText("Apakah Anda ingin mengajukan pertanyaan?");
            }else if(info.getTitle().equals("您是不是要咨询以下问题：")){
                tvTitle.setText("Apakah Anda ingin mengajukan pertanyaan?");
            } else{
    ...
    private void setRobotMenuMessageLayout(LinearLayout parentView, final RobotMenuInfo menuInfo){
        ...
        if (menuInfo.getItems() != null && !menuInfo.getItems().isEmpty()){
            ...
            for(RobotMenuInfo.Item item : items) {
                ...
                if(content.equals("返回上一级")){
                    textView.setText("Kembali ke proses pertama");
                }else if(content.equals("转人工客服")){
                    textView.setText("Hubungi customer service kami");
                }else{
